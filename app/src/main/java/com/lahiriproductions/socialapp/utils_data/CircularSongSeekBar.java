...
public class CircularSongSeekBar extends View {

	protected final float dptopxScale = getResources().getDisplayMetrics().density;
	protected final float minTouchTargetDp = 48;
	protected static final float defaultCircleXRadius = 30f;
	protected static final float defaultCircleYRadius = 30f;
	protected static final float defaultPointerRadius = 7f;
	protected static final float defaultPointerHaloWidth = 6f;
	protected static final float defaultPointerHaloBorderWidth = 2f;
	protected static final float defaultCircleStrokeWidth = 5f;
	protected static final float defaultStartAngle = 270f; // Geometric (clockwise, relative to 3 o'clock)
	protected static final float defaultEndAngle = 270f; // Geometric (clockwise, relative to 3 o'clock)
	protected static final int defaultMax = 100;
	protected static final int defaultProgress = 0;
	protected static final int defaultCircleColor = Color.DKGRAY;
	protected static final int defaultCircleProgressColor = Color.argb(235, 74, 138, 255);
	protected static final int defaultPointerColor = Color.argb(235, 74, 138, 255);
	protected static final int defaultPointerHaloColor = Color.argb(135, 74, 138, 255);
	protected static final int defaultPointerHaloColorOntouch = Color.argb(135, 74, 138, 255);
	protected static final int defaultCircleFillColor = Color.TRANSPARENT;
	protected static final int defaultPonterAlpha = 135;
	protected static final int defaultPonterAlphaOntouch = 100;
	protected static final boolean defaultUseCustomRadii = false;
	protected static final boolean defaultMaintainEqualCirlce = true;
	protected static final boolean defaultMoveOutsideCirlce = false;
	protected static final boolean defaultLockEnabled = true;
	...
	protected int mPointerColor = defaultPointerColor;
	protected int mPointerHaloColor = defaultPointerHaloColor;
	protected int mPointerHaloColorOnTouch = defaultPointerHaloColorOntouch;
	protected int mCircleColor = defaultCircleColor;
	protected int mCircleFillColor = defaultCircleFillColor;
	protected int mCircleProgressColor = defaultCircleProgressColor;
	protected int mPointerAlpha = defaultPonterAlpha;
	protected int mPointerAlphaOnTouch = defaultPonterAlphaOntouch;
	...
	protected void initAttributes(TypedArray attrArray) {
		mCircleXRadius = attrArray.getDimension(R.styleable.CircularSeekBar_circle_x_radius, defaultCircleXRadius * dptopxScale);
		mCircleYRadius = attrArray.getDimension(R.styleable.CircularSeekBar_circle_y_radius, defaultCircleYRadius * dptopxScale);
		mPointerRadius = attrArray.getDimension(R.styleable.CircularSeekBar_pointer_radius, defaultPointerRadius * dptopxScale);
		mPointerHaloWidth = attrArray.getDimension(R.styleable.CircularSeekBar_pointer_halo_width, defaultPointerHaloWidth * dptopxScale);
		mPointerHaloBorderWidth = attrArray.getDimension(R.styleable.CircularSeekBar_pointer_halo_border_width, defaultPointerHaloBorderWidth * dptopxScale);
		mCircleStrokeWidth = attrArray.getDimension(R.styleable.CircularSeekBar_circle_stroke_width, defaultCircleStrokeWidth * dptopxScale);

		mPointerColor = attrArray.getColor(R.styleable.CircularSeekBar_pointer_color, defaultPointerColor);
		mPointerHaloColor = attrArray.getColor(R.styleable.CircularSeekBar_pointer_halo_color, defaultPointerHaloColor);
		mPointerHaloColorOnTouch = attrArray.getColor(R.styleable.CircularSeekBar_pointer_halo_color_ontouch, defaultPointerHaloColorOntouch);
		mCircleColor = attrArray.getColor(R.styleable.CircularSeekBar_circle_color, defaultCircleColor);
		mCircleProgressColor = attrArray.getColor(R.styleable.CircularSeekBar_circle_progress_color, defaultCircleProgressColor);
		mCircleFillColor = attrArray.getColor(R.styleable.CircularSeekBar_circle_fill, defaultCircleFillColor);

		mPointerAlpha = Color.alpha(mPointerHaloColor);

		mPointerAlphaOnTouch = attrArray.getInt(R.styleable.CircularSeekBar_pointer_alpha_ontouch, defaultPonterAlphaOntouch);
		if (mPointerAlphaOnTouch > 255 || mPointerAlphaOnTouch < 0) {
			mPointerAlphaOnTouch = defaultPonterAlphaOntouch;
		}

		mMax = attrArray.getInt(R.styleable.CircularSeekBar_max, defaultMax);
		mProgress = attrArray.getInt(R.styleable.CircularSeekBar_progress, defaultProgress);
		mCustomRadii = attrArray.getBoolean(R.styleable.CircularSeekBar_use_custom_radii, defaultUseCustomRadii);
		mMaintainEqualCircle = attrArray.getBoolean(R.styleable.CircularSeekBar_maintain_equal_circle, defaultMaintainEqualCirlce);
		mMoveOutsideCircle = attrArray.getBoolean(R.styleable.CircularSeekBar_move_outside_circle, defaultMoveOutsideCirlce);
		lockEnabled = attrArray.getBoolean(R.styleable.CircularSeekBar_lock_enabled, defaultLockEnabled);

		// Modulo 360 right now to avoid constant conversion
		mStartAngle = ((360f + (attrArray.getFloat((R.styleable.CircularSeekBar_start_angle), defaultStartAngle) % 360f)) % 360f);
		mEndAngle = ((360f + (attrArray.getFloat((R.styleable.CircularSeekBar_end_angle), defaultEndAngle) % 360f)) % 360f);
	...
	protected void initPaints() {
		...
		mCircleProgressGlowPaint.setMaskFilter(new BlurMaskFilter((5f * dptopxScale), BlurMaskFilter.Blur.NORMAL));
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		float minimumTouchTarget = minTouchTargetDp * dptopxScale; // Convert minimum touch target into px
	...
	public CircularSongSeekBar(Context context) {
		super(context);
		init(null, 0);
	}

	public CircularSongSeekBar(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(attrs, 0);
	}

	public CircularSongSeekBar(Context context, AttributeSet attrs, int defStyle) {
	...
	*/
	public interface OnCircularSeekBarChangeListener {
		...
		public abstract void onProgressChanged(CircularSongSeekBar circularSongSeekBar, int progress, boolean fromUser);

		public abstract void onStopTrackingTouch(CircularSongSeekBar seekBar);

		public abstract void onStartTrackingTouch(CircularSongSeekBar seekBar);
	}
